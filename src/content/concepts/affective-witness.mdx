...
The Affective Witness is the chapter's load-bearing claim: felt valence is not an added phenomenal commentary on the interoceptive architecture — it is the architecture's own inside at this stratum. The three prerequisites are jointly sufficient to explain why there is something it is like to be in a body with this architecture, without invoking any additional explanatory gap. The felt weight of a situation is the precision-weighted prediction error broadcast through the distributed cognitive apparatus — not represented by it, not correlated with it, but instantiated by it.
